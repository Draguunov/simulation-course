...
### Выводы
Шаг моделирования — самый важный параметр. Если взять слишком большой шаг (например, 1 с), расчёт сразу же выдаёт ошибку: координата y становится отрицательной уже на первом шаге, и моделирование обрывается. При уменьшении шага точность растёт: при 0,1 с результаты уже заметно лучше, а при 0,01 с, 0,001 с и 0,0001 с они практически одинаковы. Поэтому оптимальным для этой задачи является шаг 0,01 с — он даёт высокую точность и при этом считает достаточно быстро.
